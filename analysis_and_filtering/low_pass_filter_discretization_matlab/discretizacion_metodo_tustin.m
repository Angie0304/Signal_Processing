% Constantes
            ...
            % Senal de entrada
            U_i = zeros(1, n_max);
            U_i(10:end) = 1;         % Paso inicial de entrada comenzando en k=10

            % Senal de salida
            U_o = zeros(1, n_max);

            % Pre computacion de denominador (constante)
            denominator = - T^2 * omega_c^2 + 2 * T * k * omega_c - 6 * T *omega_c - 4;

            % Simulacion iterativa para el sistema
            for n = 3:n_max
                numerator = 2 * T^2 * U_o(n-1) * omega_c^2 ...
                          + T^2 * U_o(n-2) * omega_c^2 ...
                          - T^2 * k * U_i(n) * omega_c^2 ...
                          - 2 * T^2 * k * U_i(n-1) * omega_c^2 ...
                          - T^2 * k * U_i(n-2) * omega_c^2 ...
                          + 2 * T * k * U_o(n-2) * omega_c ...
                          - 6 * T * U_o(n-2) * omega_c ...
                          - 8 * U_o(n-1) ...
                          + 4 * U_o(n-2);
            ...
            title('Tustin Output Simulation')
            grid on